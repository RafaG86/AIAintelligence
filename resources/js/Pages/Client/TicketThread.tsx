...
import { Cpu, Terminal, HelpCircle, ArrowLeft, Send, Loader2, Sparkles, MessageSquare } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-[#090d16] text-[#e5e7eb] flex flex-col md:flex-row font-sans">
            ...
            <main className="flex-grow flex flex-col justify-between h-screen overflow-hidden">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex gap-2 mt-1">
                                ...
                                <span className={`text-[8px] px-1.5 py-0.5 rounded uppercase font-semibold border ${
                ...
                <div className="flex-grow p-6 overflow-y-auto space-y-4 bg-[#070b13] scrollbar-thin">
                    {ticket.replies.map((reply) => {
                        const isMe = reply.user_id === auth.user.id;
                        
                        // Check if the message is from our virtual AI Support Bot
                        ...
                        return (
                            ...
                            >
                                ...
                                {isMe ? (
                                    /* Client Message Bubble (Cyan) */
                                    <div className="bg-cyan-500 text-slate-950 rounded-2xl rounded-tr-none px-4 py-3 text-sm shadow-md font-medium leading-relaxed">
                                        {reply.message}
                                    </div>
                                ) : isAi ? (
                                    /* AI Support Bot Message Bubble (Glow Violet) */
                                    <div className="bg-gradient-to-tr from-purple-950 via-[#1e1336] to-indigo-950 border border-purple-500/40 text-purple-200 rounded-2xl rounded-tl-none px-4 py-3 text-sm shadow-lg shadow-purple-500/10 leading-relaxed relative overflow-hidden neon-glow-purple">
                                        ...
                                        <div className="font-semibold text-purple-300 text-xs mb-1.5 flex items-center gap-1 uppercase tracking-wider">
                                            <span>Soporte IA Autónomo</span>
                                        </div>
                                        {reply.message}
                                    </div>
                                ) : (
                                    /* Human Admin Message Bubble (Slate) */
                ...
                <div className="bg-[#0c1221] border-t border-white/5 p-4 z-10">
                    {ticket.status === 'closed' ? (
                        <div className="text-center text-slate-500 text-xs py-3 select-none">
                            Este ticket está cerrado. Abre un nuevo ticket de soporte si requieres más ayuda.
                        </div>
                    ) : (
                        <form onSubmit={handleSendReply} className="flex gap-3 max-w-4xl mx-auto items-end">
                            <textarea
                                value={data.message}
                                onChange={(e) => setData('message', e.target.value)}
                                rows={1}
                                placeholder="Escribe tu mensaje técnico o duda..."
                            ...
                            <button
                                type="submit"
                                disabled={processing}
                                className="bg-cyan-500 hover:bg-cyan-400 disabled:bg-cyan-700/60 disabled:text-slate-400 text-slate-950 font-bold p-3.5 rounded-xl transition duration-200 cursor-pointer shadow-md shadow-cyan-500/10 shrink-0"
                            >
                                {processing ? (
